...
-- no indexes) for tables owned by `impresspress/legalpages`. CREATE TABLE
...
CREATE TABLE IF NOT EXISTS impresspress__legalpages__documents (
    id            TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_legalpages_documents_doc_type_status
    ON impresspress__legalpages__documents (doc_type, status);
