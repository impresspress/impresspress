...
-- no indexes) for tables owned by `impresspress/messages`. CREATE TABLE IF
...
CREATE TABLE IF NOT EXISTS impresspress__messages__contexts (
    id            TEXT PRIMARY KEY,
    type          TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_messages_entries_context_id_created_at
    ON impresspress__messages__entries (context_id, created_at);
CREATE INDEX IF NOT EXISTS idx_messages_entries_context_id
    ON impresspress__messages__entries (context_id);
CREATE INDEX IF NOT EXISTS idx_messages_entries_context_id_kind
    ON impresspress__messages__entries (context_id, kind);
CREATE INDEX IF NOT EXISTS idx_messages_entries_kind
    ON impresspress__messages__entries (kind);
