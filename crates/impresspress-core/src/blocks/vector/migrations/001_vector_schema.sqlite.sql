...
-- names are user-supplied at runtime (`impresspress__vector__{user_name}`) and
...
CREATE TABLE IF NOT EXISTS impresspress__vector__registry (
...
CREATE INDEX IF NOT EXISTS idx_vector_registry_model
    ON impresspress__vector__registry (model);
